...
export default function EditArticlePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      authenticatedFetch(`/api/admin/articles/${params.id}`),
      authenticatedFetch('/api/admin/categories')
    ])
    .then(async ([articleRes, categoriesRes]) => {
      if (!articleRes.ok) {
  ...
  const handlePreview = () => {
    // Create a preview with current editing content
    const previewData = {
      title: title.trim(),
      slug: slug.trim(),
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await authenticatedFetch(`/api/admin/articles/${params.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        showStatus(`ARTICLE UPDATED SUCCESSFULLY - View at: /article/${slug.trim()}`, 'success');
        ...
        if (errorData.error === 'Slug already exists') {
          ...
          showStatus(`ERROR: ${errorData.error || 'Failed to update article'}`, 'error');
        }
      }
    } catch (error) {
  ...
  if (loading) {
    return (
      <AuthenticatedLayout titlePrefix="Edit Article" currentPage="edit">
        <div className="p-8">
...
Your content goes here."
            ...
            <div className="space-y-6">
              ...
              {/* Article Info */}
              <div className="bloomberg-panel">
                <h3 className="text-lg font-bold terminal-orange mb-4">METADATA</h3>
                <div className="space-y-2 text-sm">
                  <div>ID: <span className="terminal-green">{article.id}</span></div>
                  <div>CREATED: <span className="terminal-green">{new Date(article.created_at).toLocaleDateString()}</span></div>
                  {article.published_at && (
                    <div>PUBLISHED: <span className="terminal-green">{new Date(article.published_at).toLocaleDateString()}</span></div>
                  )}
                </div>
              </div>
              ...
              <div className="bloomberg-panel">
                ...
                <div className="space-y-2">
                  <button
                    onClick={handleSave}
                    disabled={saving}
                    className="flex items-center gap-2 bloomberg-button disabled:opacity-50 w-full justify-center"
                  >
                    <Save size={16} />
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2 px-4 border border-bloomberg-green text-bloomberg-green hover:bg-bloomberg-green hover:text-black transition-colors"
                  >
                  ...
                  <Link
                    href="/"
                    className="block w-full text-center py-2 px-4 border border-bloomberg-gray text-bloomberg-gray hover:bg-bloomberg-gray hover:text-black transition-colors"
                  >
                    CANCEL / BACK
                  </Link>
                </div>
